...
teamInfo.forEach((teamMember) => {
  const item = document.createElement("div");
  item.classList.add("tea-grid-item");
  if (teamMember.tea) {
    const svg = createTeaSvg(teamMember.color);
    const rest = `<p class="tea-text">${teamMember.name}</p>`;
    item.innerHTML = svg + rest;
    teaGrid.appendChild(item);
  } else {
    const svg = createCoffeeSvg();
    const rest = `<p class="tea-text coffee">${teamMember.name}</p>`;
    item.innerHTML = svg + rest;
    teaGrid.appendChild(item);
  }
});
